...
vacci_rate=[0.0005 0.001 0.005 0.01];    % immunization rate due to vaccination

mbirth_rate=0.01;   % mosquito birth rate
mdeath_rate=0.01;   % mosquito death rate

for j = 1:length(vacci_rate)
    ...
    for i=1:N
        ...
        sus_humans(i+1)=sus_humans(i)+ dt*(-(( sus_humans(i)*inf_mosq(i)*pbite )/ (sus_mosq(i)+inf_mosq(i)) ) + reco_rate*inf_humans(i) - vacci_rate(j)*sus_humans(i)) ;  % /N needed?
        ...
        immune_humans(i+1)= immune_humans(i) + dt*(immune_rate*inf_humans(i) + vacci_rate(j)*sus_humans(i)) ;

        sus_mosq(i+1)= sus_mosq(i) + dt*(-(( sus_mosq(i)*inf_humans(i)*pbite ) /(total_humans)) + (mbirth_rate)*(sus_mosq(i)+inf_mosq(i)) - (mdeath_rate)*(sus_mosq(i))) ;

        inf_mosq(i+1)= inf_mosq(i) + dt*( ( sus_mosq(i)*inf_humans(i)*pbite )/(total_humans) - (mdeath_rate)*(inf_mosq(i)) ) ;
    ...
    title('Effect of changing vaccination rate(vr) on susceptible humans');
    xlabel('time (in days)','FontSize',16);
    ylabel('Number of humans','FontSize',16);
    legend('vr = 0.0005', 'vr = 0.001', 'vr = 0.005', 'vr = 0.01');

    hold on
    subplot(2,2,2), plot(time,inf_humans,'LineWidth', 2);
    title('Effect of changing vaccination rate(vr) on infected humans');
    xlabel('time (in days)','FontSize',16);
    ylabel('Number of humans','FontSize',16);
    legend('vr = 0.0005', 'vr = 0.001', 'vr = 0.005', 'vr = 0.01');
    ...
    title('Effect of changing vaccination rate(vr) on dead humans');
    xlabel('time (in days)','FontSize',16);
    ylabel('Number of humans','FontSize',16);
    legend('vr = 0.0005', 'vr = 0.001', 'vr = 0.005', 'vr = 0.01');
    ...
    title('Effect of changing vaccination rate(vr) on immune humans');
    xlabel('time (in days)','FontSize',16);
    ylabel('Number of humans','FontSize',16);
    legend('vr = 0.0005', 'vr = 0.001', 'vr = 0.005', 'vr = 0.01');
...
%     title('Effect of changing vaccination rate(vr) on susceptible mosquitoes');
%     xlabel('time (in days)','FontSize',16);
...
%     title('Effect of changing vaccination rate(vr) on infected mosquitoes');
%     xlabel('time (in days)','FontSize',16);
%     ylabel('Number of mosquitoes','FontSize',16);
%     legend('vr = 0.0005', 'vr = 0.001', 'vr = 0.005', 'vr = 0.01');
